...
A 3D view of the open-source dependency ecosystem: 7,051 GitHub repositories
placed in a cylinder where **height** is how deep a repo sits in the
dependency stack, **angle** is its subject matter, and **radius** is how
focused that subject is — then linked by five kinds of real, measured
relationship.

Built from the GitHub API and [SemRepo](https://semrepo.org), a large
RDF/N-Triples dump of GitHub activity (repos, issues, stargazers, forks,
contributors, package usage). Sizing that honestly, because it is easy to
overstate either side: SemRepo supplies 31% of the drawn edges and is the
*only* source for the three person-based tiers, while the cohort itself,
descriptions, topics, READMEs, avatars and five of the six dependency-manifest
ecosystems come from the GitHub API. SemRepo is the upstream dataset, not this
project and not its publisher.

**The name.** The pelagic zone is the open water column, read by depth rather
than drawn as a map. The vertical axis here is a trophic solve
(`scripts/15_trophic_levels.py`, using MacKay/Johnson/Sansom's
trophic-coherence formalism for food webs) — consumers sit above what they
consume. That is the same mathematics marine ecologists use on food webs, and
for the same reason it is averaged rather than path-counted: real dependency
graphs, like real food webs, contain cycles.
