...
text embeddings (scripts/17_fetch_readmes.py, scripts/18_text_embeddings.py)
...
Usage: python3 scripts/14_cluster_hierarchy.py
"""
import json
import math
import re
...
# This codebase's usual min-shared-support threshold (see compute_shared_edges.py's
...
def pmi_edges(item_by_doc, min_item_support, min_pair_support):
    ...
    Same shape as scripts/16_topic_circular_embedding.py's build_topic_graph,
...
def build_topic_pmi_edges(cohort):
    ...
    carrying them -- the transpose of script 16's topic-topic PMI (there,
...
def build_text_embedding_edges(cohort):
    ...
    first paragraph` per repo (scripts/17_fetch_readmes.py,
    scripts/18_text_embeddings.py) reaches 317/319 -- this is the "coverage,
    not noise" signal the Phase 14 design is named for. Missing the
    embeddings file (script 18 not run yet) degrades gracefully to the
