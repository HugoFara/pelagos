#!/usr/bin/env python3
"""Aggregate rows for the non-GitHub forge nodes 46 corroborated.
...
46_debian_dependency_edges.py) and merges rows into
...
Usage: python3 scripts/48_fetch_forge_repo_stats.py
"""
import json
import sys
import urllib.error
...
def load_forge_nodes():
    """[(node id, origin url)] from 46's output."""
...
def main():
    ...
    if not nodes:
        print("no forge_extra_nodes.txt -- run 46_debian_dependency_edges.py first",
              file=sys.stderr)
        return
