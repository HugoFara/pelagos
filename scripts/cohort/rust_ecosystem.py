...
Rust is the last big gap go_ecosystem.py's docstring named but left unfilled -- counted
...
cpp_ecosystem.py/cpp_manifests.py/cpp_deps.py; see
scripts/edges/cpp_deps.py. The one part of the original
...
real dependency edges come from Cargo.toml instead (cargo_toml.py/rust_deps.py), the
...
Usage: python3 scripts/cohort/rust_ecosystem.py [count=1000]
"""
import json
import subprocess
import sys
...
def main(count="1000"):
    ...
    max_stars = None
    total_unique = 0
    # Same star-floor continuation as java/go_ecosystem.py: a single query's
    # top-1000-by-stars window can be exhausted by cohort overlap well before
    # `count` new repos
