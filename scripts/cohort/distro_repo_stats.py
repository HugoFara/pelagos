#!/usr/bin/env python3
"""Fetch GitHub stats for the repos scripts/edges/debian_deps.py wants to add.

debian_deps.py resolves Debian's package graph to repositories and works out
which of them
...
Same shape and same cache as scripts/cohort/js_ecosystem.py: one
...
Usage: python3 scripts/cohort/distro_repo_stats.py
"""
import json
import subprocess
import sys
...
def gh_api_repo(owner, name):
    ...
    scripts/cohort/js_ecosystem.py's, including the negative cache."""
...
def aggregate_row(data):
    return {
        ...
        "contributors": None,  # not on the repos endpoint, same as cohort/*_ecosystem.py
...
def main():
    if not EXTRA_LIST.exists():
        print("no distro_extra_repos.txt -- run scripts/edges/debian_deps.py first",
              file=sys.stderr)
        return
    ...
    for repo, data in results:
        ...
        # the name it actually answers to -- the same rule scripts/identity/repo_identity.py
