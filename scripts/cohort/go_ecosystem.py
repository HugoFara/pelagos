...
scripts/cohort/python_ecosystem.py: GitHub's Search API ranked by stars,
`language:go` only.
...
PyPI-dependency-degree "source repo" stream (scripts/cohort/dependency_repos.py)
...
semantic (topic) and text-embedding edges once topic_edges.py,
repo_readmes.py and text_embeddings.py run, since those two signals are
GitHub-API-derived and don't care which stream a repo came
from.
...
Usage: python3 scripts/cohort/go_ecosystem.py [count=1000]
"""
import json
import subprocess
import sys
...
def main(count="1000"):
    ...
    max_stars = None
    total_unique = 0
    # Same star-floor continuation as scripts/cohort/java_ecosystem.py: a single
