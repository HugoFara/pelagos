...
scripts/identity/repo_identity.py worked out.

repo_identity.py decides *which* slugs name the same repository. This applies that decision
...
numbers:
  ...
  - **Keyed by repo** (aggregates, descriptions, topics, embeddings). The
    ...
    expansion -- and scripts/build/web_explorer.py merges them with the expansion
...
Usage: python3 scripts/identity/apply_identity.py [--dry-run]
"""
import json
import sys
...
from lib.identity import alias_map, canonical  # noqa: E402

ROOT = Path(__file__).resolve().parents[2]
PROCESSED = ROOT / "data/processed"
REPO_LISTS = ROOT / "data/repo-lists"

# The cohort's node set, in the order scripts/build/web_explorer.py merges them: the
...
def merge_aggregates(dry_run):
    ...
    scripts/build/web_explorer.py's `update()` order stops mattering."""
