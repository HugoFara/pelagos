...
Java repos (scripts/25_fetch_java_ecosystem_repos.py) currently have zero
dependency edges, the same gap go.mod closed for Go (scripts/28/29) and
package.json closed for JS (scripts/30/31). Java has no single canonical
...
33_java_dependency_edges.py knows how to parse it without re-probing. A repo
...
measured, not assumed -- running 33's own parser over all 811 cached root
...
33_java_dependency_edges.py, so that exclusion can be measured and changed
...
Usage: python3 scripts/32_fetch_java_manifests.py
"""
import base64
import json
import subprocess
