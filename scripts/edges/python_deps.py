...
scripts/lib/manifests.py -- not one file at the root of the repos GitHub's
...
Fifth and last ecosystem folded into scripts/edges/dependency_edges.py's combined
...
Resolution reuses what scripts/edges/resolve_pypi_packages.py already built: the same
...
setup.py is not read at all; see python_manifests.py's docstring for why (two
repos, and the alternatives are executing repo code or AST-guessing a non-literal).
...
cache path is still checked first so resolve_pypi_packages.py's existing entries stay warm.

Output shape matches go/js/java/rust_deps.py: [source, target, weight, [package names]],
weight = how many distinct packages of that target this repo depends on.

Usage: python3 scripts/edges/python_deps.py [out=data/processed/repo_python_dependency_edges.json]
"""
import json
import re
import sys
...
PYPI_CACHE = ROOT / "data/raw/pypi_cache"  # shared with scripts/edges/resolve_pypi_packages.py
PYPI_FETCH_THROTTLE_S = 0.1  # same margin scripts/edges/resolve_pypi_packages.py uses
...
def fetch_pypi_repo(package):
    """owner/repo for a PyPI project, or None. Reuses scripts/edges/resolve_pypi_packages.py's
...
def resolve_from_pypi_json(data):
    """Same walk scripts/edges/resolve_pypi_packages.py uses: preferred project_urls keys
