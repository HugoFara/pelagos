...
on--> repo` edges, the Go-ecosystem counterpart to scripts/edges/dependency_edges.py's
...
- `replace`/`exclude`/`retract` directives need no special handling: the
  same target==source self-loop filter scripts/edges/dependency_edges.py already uses
...
Output shape mirrors scripts/edges/dependency_edges.py's full-edge tuples (source,
...
Usage: python3 scripts/edges/go_deps.py [out=data/processed/repo_go_dependency_edges.json]
"""
import json
import re
import sys
...
def main(out_path="data/processed/repo_go_dependency_edges.json"):
    ...
    # itself, same reasoning scripts/edges/resolve_pypi_packages.py's package_to_repo.json
