...
scripts/edges/dependency_edges.py folds it into the one shared prune pass; plus
...
scripts/cohort/distro_repo_stats.py to fetch stats for).

Usage: python3 scripts/edges/debian_deps.py [min_reverse_deps=5]
"""
import json
import lzma
import re
...
def main(min_reverse_deps=MIN_REVERSE_DEPS):
    ...
    # scripts/cohort/distro_repo_stats.py to add, and only becomes an endpoint on
    ...
    # aggregates. scripts/build/web_explorer.py caught it -- those endpoints could not
