...
on--> repo` edges, the JS/TS-ecosystem counterpart to scripts/edges/dependency_edges.py's
PyPI-usedPackage-based edges and scripts/edges/go_deps.py's go.mod-based
ones (see scripts/fetch/package_json.py for why package.json needed no
...
registry lookup -- same shape as scripts/edges/resolve_pypi_packages.py's PyPI resolution,
...
signal or none" rule go_deps.py already applies to non-GitHub Go vanity domains.
...
"depends on" edge, the npm parallel of go_deps.py only counting go.mod's non-`//
indirect` requires.
...
mattered and both were measured (see scripts/lib/manifests.py): workspace
...
Output shape mirrors dependency_edges.py's / go_deps.py's full-edge tuples
(source, target, weight, [package names]) so dependency_edges.py can fold this
file straight
...
Usage: python3 scripts/edges/js_deps.py [out=data/processed/repo_js_dependency_edges.json]
"""
import json
import re
import sys
...
NPM_FETCH_THROTTLE_S = 0.1  # same politeness margin scripts/edges/resolve_pypi_packages.py uses against PyPI's JSON API
