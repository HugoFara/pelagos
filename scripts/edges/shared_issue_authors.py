...
hasStargazer/hasContributor (shared_stargazers.sh/shared_contributors.sh) --
hasIssueAuthor is exactly the same issue/repo->person triple shape -- but
resolved via a two-hop join instead of a direct grep, since hasIssueAuthor's
subject is the *issue*, not the repo (`<repository/{owner}/{repo}/issue/{n}> hasIssueAuthor person/{who}`).
...
not full neighborhood" idiom scripts/extract/repo_expansions.sh already uses for
...
encountered in file order, deterministic, same convention as
repo_expansions.sh's MAX_ISSUE) keeps the two-hop join's working set small
(well under 3000 sampled issues total) while still giving every covered repo a real, if partial, sample.
...
scripts/edges/resolve_pypi_packages.py) rather than shelling out via a separate .sh script,
since the cross-predicate join/cap logic doesn't reduce to a single grep|awk
one-liner the way extract/'s simpler single-predicate passes do.

Usage: SEMREPO_NT=/path/to/SemRepo.nt python3 scripts/edges/shared_issue_authors.py \
...
from lib.ntparse import parse_line, short_predicate, repo_name  # noqa: E402
...
def main(repo_list_path, out_authors_nt="data/raw/repo_issue_authors_full.nt",
    ...
    # '>' -- same PATTERNS shape shared_stargazers.sh/shared_contributors.sh
    # use -- so this can't also pick up pass 2's issue-subject lines below).
