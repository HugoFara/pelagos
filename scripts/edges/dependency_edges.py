...
GitHub repo, from scripts/edges/resolve_pypi_packages.py) and dependency_source_id_map.json
(dependency-cohort raw id -> canonical owner/repo, from
scripts/cohort/dependency_repos.py -- those ids are inconsistently ordered, see
NOTES.md).

Only emits an edge when both ends are real nodes (the 51 original +
whatever scripts/cohort/dependency_repos.py added). Matching is case-insensitive
...
Shape mirrors scripts/lib/shared_edges.py's with_members=1 output (source,
...
derived edges, see scripts/edges/go_deps.py), data/processed/
repo_js_dependency_edges.json (JS/TS's package.json-derived edges, see
scripts/edges/js_deps.py), data/processed/repo_java_dependency_edges.json
(Java's pom.xml/build.gradle-derived edges, see scripts/edges/java_deps.py),
data/processed/repo_rust_dependency_edges.json (Rust's Cargo.toml-derived
edges, see scripts/edges/rust_deps.py), and data/processed/
...
scripts/edges/python_deps.py -- those repos postdate the SemRepo dump, so
...
-derived edges, see scripts/edges/cpp_deps.py), and data/processed/
repo_debian_dependency_edges.json (Debian's binary `Depends` field, see
scripts/edges/debian_deps.py -- the only source here that crosses the
...
scripts/lib/shared_edges.py prunes (keep an edge if it's in either endpoint's
...
Usage: python3 scripts/edges/dependency_edges.py [out_full=data/processed/repo_dependency_edges.json]
...
def package_target_cache(package_to_repo, canon_lookup):
    ...
    API responses scripts/cohort/dependency_repos.py already fetched for every
...
def main(out_full="data/processed/repo_dependency_edges.json",
    ...
    # scripts/edges/go_deps.py; JS/TS's package.json, scripts/edges/js_deps.py;
    # Java's pom.xml/build.gradle, scripts/edges/java_deps.py; Rust's
    # Cargo.toml, scripts/edges/rust_deps.py; Python's pyproject.toml/
    # requirements.txt, scripts/edges/python_deps.py), if that
    ...
    # scripts/layout/text_embeddings.py uses for ISSUE_TITLES_PATH.
    extra_sources = [
    ...
    # top-K list, same shape as scripts/lib/shared_edges.py's pruning, but
