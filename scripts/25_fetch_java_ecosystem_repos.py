...
24_fetch_js_ecosystem_repos.py grew it with JS/TS ones: the PyPI-dependency-degree
...
/ data/repo-lists/dependency_extra_repos.txt files 10_fetch_new_repo_stats.py
and 24_fetch_js_ecosystem_repos.py write, so every downstream script (12/13/14/17/18,
build_web_explorer.py) picks these repos up with zero changes -- they all
already read "top50 + dependency_extra_repos.txt" as the full cohort. Like
script 24, deliberately does NOT touch data/processed/dependency_source_id_map.json:
these repos have no real usedPackage backing, so 11_dependency_edges.py
...
that downstream pipeline is run separately, same as 24 left things after
a34c676 and before ec3818c.

Usage: python3 scripts/25_fetch_java_ecosystem_repos.py [count=1000]
"""
import json
import subprocess
import sys
