...
11_dependency_edges.py's PyPI-based edges, 29_go_dependency_edges.py's
go.mod-based ones, and 31_js_dependency_edges.py's package.json-based ones
(see scripts/32_fetch_java_manifests.py for why Java needed a 3-way
...
- Maven: a `groupId:artifactId` coordinate carries no repo-hosting
  ...
  than guessed, the direct Java-ecosystem analogue of 29's golang.org/x/*
  and gopkg.in/* gap.
...
Output shape mirrors 11/29/31's full-edge tuples (source, target, weight,
[coordinates]) so 11 can fold this file straight into its own combined top-K
...
Usage: python3 scripts/33_java_dependency_edges.py [out=data/processed/repo_java_dependency_edges.json]
"""
import json
import re
import sys
...
def main(out_path="data/processed/repo_java_dependency_edges.json"):
    ...
    # of the whole pipeline. Same reasoning as 32's threaded sweep: the work is
    # latency-bound and every coordinate writes only its own cache file, so a
    # small pool is safe. Deliberately smaller than 32's -- Maven Central is a
