#!/usr/bin/env python3
"""Turn a repo-stargazer bipartite edge list (from scripts/extract/shared_stargazers.sh) into a
...
Usage: python3 scripts/lib/shared_edges.py raw/repo_stargazers_full.nt processed/repo_shared_edges.json processed/repo_shared_edges_pruned.json [min_shared] [top_k] [with_members]
...
shared-issue-author); leave it 0 for scripts/edges/topic_edges.py, whose "members"
...
SALT_PATH = Path(__file__).resolve().parents[2] / "data/raw/pseudonym_salt.txt"
...
def build_edges(bipartite, min_shared=3, top_k=4, with_members=0):
    ...
    edges built straight from cached JSON -- see scripts/edges/topic_edges.py).
    """
