...
scripts/identity/repo_identity.py works out which slugs are the same repository (renames,
...
every caller keeps working on a checkout where repo_refs.py/repo_identity.py
have never been run --
the same convention scripts/edges/dependency_edges.py uses for the per-ecosystem edge
files.
"""
import json
