...
Until now each ecosystem's fetcher (scripts/fetch/{go_mod,package_json,
java_manifests,cargo_toml,python_manifests,cpp_manifests}.py) read exactly one
...
`scripts/fetch/repo_manifests.py` reads one tree per cohort repo and caches every
...
scripts/fetch/java_manifests.py already set for `buildSrc/`: the cache records
...
scripts/edges/dependency_edges.py uses for the per-ecosystem edge files.
"""
import hashlib
import json
...
# manifests than scripts/fetch/repo_manifests.py's fetch budget, so the budget is
...
def content_hash(text):
    ...
    the intrinsic git object ids scripts/identity/repo_refs.py reads, applied one
...
def copied_hashes(inventory):
    ...
    for repo, by_eco in inventory.items():
        ...
        for paths in by_eco.values():
            ...
            # scripts/fetch/repo_manifests.py to regenerate it (no re-fetching: the
...
def copied_hash_set():
    ...
    for a checkout where scripts/fetch/repo_manifests.py has never run: with no
