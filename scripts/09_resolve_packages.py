...
Usage: SEMREPO_NT=/path/to/SemRepo.nt python3 09_resolve_packages.py \
...
def distinct_packages(nt_path):
    ...
    # e.g. find_repos_with_packages.sh, which never run -P over the raw dump).
...
def main(nt_path=None, cache_dir="data/raw/pypi_cache", out_path="data/processed/package_to_repo.json"):
    ...
    if not nt_path and not cached_subset.exists():
        sys.exit("usage: SEMREPO_NT=/path/to/SemRepo.nt python3 09_resolve_packages.py "
                 "[pypi_cache_dir] [out]")
