...
monoculture the PyPI-dependency-driven stream (10_fetch_new_repo_stats.py)
...
writes, so every downstream script (12/13/14/17/18, build_web_explorer.py)
...
usedPackage backing, so 11_dependency_edges.py correctly leaves them
...
13/17/18 run, since those two signals are GitHub-API-derived, not
dump-derived, and don't care which stream a repo came from.

Usage: python3 scripts/24_fetch_js_ecosystem_repos.py [count=1000]
"""
import json
import subprocess
import sys
