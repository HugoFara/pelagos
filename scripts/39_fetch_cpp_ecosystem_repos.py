...
34_fetch_rust_ecosystem_repos.py: GitHub's Search API ranked by stars.
...
## Correcting the record in 34's docstring

34_fetch_rust_ecosystem_repos.py claims C/C++'s dependency signal
...
see 41_cpp_dependency_edges.py for which parts of it are actually
...
real dependency edges come from scripts 40/41 instead.
...
Usage: python3 scripts/39_fetch_cpp_ecosystem_repos.py [count=1000]
"""
import json
import subprocess
import sys
...
def main(count="1000"):
    ...
    exhausted = set()
    total_unique = 0
    # Same star-floor continuation as 25/27/34, run per language: a single
