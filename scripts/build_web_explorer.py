...
Usage: python3 build_web_explorer.py [processed_dir] [template] [out]
...
def main(processed_dir="data/processed", template_path="web/template.html", out_path="web/index.html"):
    ...
    # Real repo-depends-on-repo edges (10_fetch_new_repo_stats.py /
    # 11_dependency_edges.py) -- grows the 51-repo cohort with the GitHub
    ...
    # Pre-fetched GitHub descriptions (12_cache_repo_descriptions.py), inlined
    ...
    # Semantic edges (13_semantic_edges.py) -- two repos sharing GitHub topic
    ...
    # a capped per-repo issue sample (scripts/23_shared_issue_authors.py) --
    ...
    # Precomputed multi-level cluster hierarchy (14_cluster_hierarchy.py) --
    ...
    # Phase 12 coordinate system: trophic height (15_trophic_levels.py, the
    ...
    # 08_download_repo_logos.py is re-run, and permanently for any owner
    # whose account is gone (08 recovers renames via the API, not deletions).
    ...
    # scripts 13/15/16 run over the cohort rather than approximating them.
    #
    # 1. topic -> angle, straight from 16_topic_circular_embedding.py's
    ...
    #    tables scripts 29/31/33/36/38/41 built from real registry lookups.
    #    Shipping them means a looked-up repo's manifest resolves with zero
    #    extra network calls. Measured on non-cohort repos: these captured
    ...
    # The PyPI table 09_resolve_packages.py built from the dump's usedPackage
    # triples covers names 38_python_dependency_edges.py never had to resolve;
    ...
    if missing_logos:
        print(f"{len(missing_logos)} owner(s) have no web/logos/*.png and are marked "
              ...
              f"08_download_repo_logos.py to close real gaps: {missing_logos[:10]}", file=sys.stderr)
