...
scripts/14_cluster_hierarchy.py mints cluster ids positionally
...
scripts/14_cluster_hierarchy.py's real_clusters budget check), but the
hierarchy format supports it and this should too.

Labels are deliberately NOT touched here -- scripts/22_label_clusters.py
...
disagree with 22's own cache.

Usage: python3 scripts/21_stabilize_cluster_ids.py
"""
import json
import re
import sys
...
def main():
    ...
    if not PREV_PATH.exists():
        print(
            ...
            f"({len(new_hier['clusters'])} clusters keep scripts/14's positional ids)",
            file=sys.stderr,
        )
