...
The 68 repos from scripts/cohort/python_ecosystem.py are the last cohort
...
usedPackage triples scripts/edges/dependency_edges.py reads come from the SemRepo dump,
...
same read-the-manifest treatment Go (go_mod.py/go_deps.py), JS
(package_json.py/js_deps.py), Java (java_manifests.py/java_deps.py) and Rust
(cargo_toml.py/rust_deps.py) got.
...
while shipping a populated requirements.txt. scripts/edges/python_deps.py
...
kind is whichever file existed, so scripts/edges/python_deps.py knows how to
...
Usage: python3 scripts/fetch/python_manifests.py
"""
import base64
import json
import subprocess
