...
This is the fetch half of the multi-ecosystem change; `scripts/lib/manifests.py`
...
Generalizes the submodule sweep scripts/fetch/java_manifests.py already runs for
...
mechanics are java_manifests.py's, unchanged and for its reasons -- a `git/trees/HEAD`
...
The two differ for two honest reasons, each counted separately in the cache:
  ...
  - vendored paths (`node_modules/`, `third_party/`, a checked-in `vendor/`)
    ...
    the exclusion rule lives in scripts/lib/manifests.py where it can be changed
...
has no manifests" -- java_manifests.py's rule, and it matters more here since this pass is
...
Usage: python3 scripts/fetch/repo_manifests.py [limit=0]
...
from lib.manifests import (  # noqa: E402
...
MANIFEST_BATCH = 50  # aliased blobs per GraphQL query; see scripts/fetch/java_manifests.py
SWEEP_WORKERS = 6  # latency-bound, not rate-limit-bound -- see java_manifests.py's docstring
...
def gh_json(args):
    ...
    burst but returns immediately on a real 404 -- scripts/fetch/java_manifests.py's
    gh_json, unchanged."""
...
def build_inventory(repos):
    ...
    scripts/lib/manifests.py drops those at read time. Carrying the hash here
...
def report(inventory, repos, failed):
    ...
    print(f"{copied_files} nested manifests are byte-identical to another repo's file "
          ...
          f"scripts/lib/manifests.py -- not this repo's own declaration)", file=sys.stderr)
